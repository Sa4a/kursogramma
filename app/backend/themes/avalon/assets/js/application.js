...
var Utility = {
        ...
        // --- select2 input in modal fix
        select2_input_fix: function () {
            $('.modal').on('shown.bs.modal', function () {
                var $modal = $(this);
                var hideOnEsc = function (e) {
                    if (e.keyCode === 27) {
                        $(this).modal('hide');
                    }
                };
                $(document).off("keydown", hideOnEsc);
                if ($modal.find('[data-krajee-select2]').length) {
...
// ------------------------------
(function ($) {
    $(document).on('click', '.show_tr', function () {
        if ($(this).text() == 'Показать ещё') {
            $(this).parent().find('.hide_tr').show('slow');
            $(this).text('Скрыть всё, кроме 3 первых');
        }
        else {
            $(this).parent().find('.hide_tr').hide(100);
...
$(document).ready(function () {
    ...
    // select2 input in modal fix init
    Utility.select2_input_fix();
});
// ------------------------------
// =/D No more D for you.
// ------------------------------
