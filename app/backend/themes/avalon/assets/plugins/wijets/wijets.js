...
$.wijets = function () {
	...
	this.registerAction( {
		...
		onClick: function () {
			...
			var visible = $(this).closest('.panel').find(params.target).is(':visible');

        	$(this).closest('.panel').find(params.target).slideToggle(100, 'linear', function () {
		...
		onInit: function () {
			...
			if ($(this).getWidgetState('collapsed') == "true") {
				if (!params.target) params.target = '.panel-body';
				$(this).find('i').toggleClass('fa-minus').toggleClass('fa-plus');
				$(this).closest('.panel').find(params.target).hide();
